...
Es <- list()
test_PM <- list()
fold_df <- as.data.frame(matrix(data = NA, nrow = k + 1, ncol = 6))
names(fold_df) <- c("stations", "MAE", "MSE", "RMSE", "R2", "Bias")
...
leave_one_out_red$stations <- leave_one_out_red$stations %>% unlist()
write.csv(leave_one_out_red, file = "C:\\Users\\aboser\\Documents\\GitHub\\PM_prediction\\Data\\Final_DFs\\LME_CV_red.csv")
end_time <- Sys.time()
...
paste0("Unadjusted full two-stage model on only instances with AOD. MSE: ", tsMSE, ". R2: ", tsR2, ". RMSE: ", tsRMSE, ". MAE: ", tsMAE, ".")
```
